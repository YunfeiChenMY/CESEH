...
from load_data import get_loader_flickr, get_loader_nus, get_loader_coco, get_loader_flickr_CLIP, get_loader_nuswide_CLIP, get_loader_coco_CLIP
        ...
        for j in range(4):  # {10}:#10
            ...
            for k in range(1):  # range(10):#{100}:
                ...
                for y in range(1):  # {0.1, 0.01, 0.001}: #0.001, 0.01, 0.1
                    # l4 = y
                    l4 = 10 ** (-1 * y + 0)  # 0.001-100
                    # l4 = 2**((-1)**y*y+5)
                    # l4 = 2 ** (y - 0)
                    for i1 in range(
                            1):  # {0.3}: #0.00001, 0.0001, 0.001, 0.01, 0.1, 0.2, 0.4, 0.6, 0.8, 1, 2, 4, 6, 8, 10
                        l5 = i1 * 0.1
                            ...
                            for k1 in range(1):  # {0.1}: # 0.01, 0.1
                                ...
                                for y1 in range(100):  # {150}:
                                    ...
                                    sess = Session(train_loader, test_loader, database_loader, train_dataset,
                                                   test_dataset, database_dataset, data_train, l5, l5, l8)
