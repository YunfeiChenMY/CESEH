...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, emb_i, emb_j):
        ...
        return loss
class Session:
    def __init__(self, train_loader, test_loader, database_loader, train_dataset, test_dataset, database_dataset, data_train, l5, l8, a3):
        ...
        self.gs = self.cal_similarity(img_norm, txt_norm, a3, a3,a3)
        self.S = self.gs
    ...
    def eval(self, l1, l2, l3, l4, l5, l6, l7, l8):
        ...
        self.logger.info('MAP: %.6f, %.6f, %.6f, %.6f, %.6f, %.6f, %.6f, %.6f, %.6f, %.6f, %.6f, %.6f, %.6f' % (l1, l2, l3, l4, l5, l6, l7, l8, MAP_I2T, MAP_T2I, 0, 0, MAP_I2T+MAP_T2I))
        # self.logger.info('--------------------------------------------------------------------')
                    ...
                    for i1 in range(1):#{0.3}: #0.00001, 0.0001, 0.001, 0.01, 0.1, 0.2, 0.4, 0.6, 0.8, 1, 2, 4, 6, 8, 10
                        ...
                        for j1 in range(1):#{0.45}:
                            ...
                            for k1 in range(1):#{0.1}: # 0.01, 0.1
                                ...
                                l7 = 2**(k1+12)
                                # l7 = 2**((-1)**k1*k1+0)*1000
                                for y1 in range(100):#{150}:
                                    l8 = 20*y1 + 20
                                    sess = Session(train_loader, test_loader, database_loader, train_dataset,
                                                   test_dataset, database_dataset, data_train, l5, l8, l8)
                                    ...
                                    if settings.EVAL == True:
                                        ...
                                        for epoch in range(settings.NUM_EPOCH):
                                            ...
                                            if (epoch + 1) % settings.EVAL_INTERVAL == 0:
                                                A = sess.eval(l1, l2, l3, l4, l5, l6, l7, l8)
                                                # print(A)
                                                if A < 1.84:
                                                    break
                                            # save the model
